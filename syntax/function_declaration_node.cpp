...
FunctionDeclarationNode::FunctionDeclarationNode(
    SyntaxToken *token, std::string type_identifier, std::string function_name,
    std::vector<FormalParameterNode *> params, bool is_struct, bool is_const)
    : SyntaxNode(token), m_type_identifier(type_identifier),
      m_function_name(function_name), m_parameters(params),
      m_is_struct(is_struct), m_is_const(is_const) {
  ...
  for (auto param : params) {
    const Symbols::TypeSymbol *ptype =
        param->is_struct()
            ? new Symbols::TypeSymbol(param->type_identifier(), t)
            : Symbols::TypeSymbol::try_lookup_primitive(
                  param->type_identifier());

    m_param_symbols.push_back(new Symbols::ParameterSymbol(
        param->param_name(), ptype, param->is_array(), param->is_const()));
  }

  m_function_symbol =
      new Symbols::FunctionSymbol(function_name, m_type, m_param_symbols);
}

FunctionDeclarationNode::~FunctionDeclarationNode() {
  std::vector<FormalParameterNode *>::iterator param_iter;
  for (param_iter = m_parameters.begin(); param_iter != m_parameters.end();
       param_iter++)
    delete *param_iter;
}
 ...
 */
std::vector<Symbols::ParameterSymbol *>
FunctionDeclarationNode::parameter_symbols() const {
 ...
 */
bool FunctionDeclarationNode::is_struct() const { return m_is_struct; }

/*
 * Whether the function's return type is a constant type or not.
 */
bool FunctionDeclarationNode::is_const() const { return m_is_const; }
