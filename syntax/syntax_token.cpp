...
namespace Syntax {
SyntaxToken::SyntaxToken(std::string text, token_type_t token,
                         std::string filename, int begin_line, int begin_column,
                         int end_line, int end_column,
                         token_variant semantic_value)
    : m_text(text), m_token(token), m_filename(filename),
      m_begin_line(begin_line), m_begin_column(begin_column),
      m_end_line(end_line), m_end_column(end_column),
      m_semantic_value(semantic_value) {}

SyntaxToken::SyntaxToken(std::string text, token_type_t token,
                         std::string filename, int begin_line, int begin_column,
                         int end_line, int end_column, int i_val)
    : m_text(text), m_token(token), m_filename(filename),
      m_begin_line(begin_line), m_begin_column(begin_column),
      m_end_line(end_line), m_end_column(end_column) {
...
SyntaxToken::SyntaxToken(std::string text, token_type_t token,
                         std::string filename, int begin_line, int begin_column,
                         int end_line, int end_column, float f_val)
    : m_text(text), m_token(token), m_filename(filename),
      m_begin_line(begin_line), m_begin_column(begin_column),
      m_end_line(end_line), m_end_column(end_column) {
...
SyntaxToken::SyntaxToken(std::string text, token_type_t token,
                         std::string filename, int begin_line, int begin_column,
                         int end_line, int end_column, char char_val)
    : m_text(text), m_token(token), m_filename(filename),
      m_begin_line(begin_line), m_begin_column(begin_column),
      m_end_line(end_line), m_end_column(end_column) {
...
SyntaxToken::SyntaxToken(std::string text, token_type_t token,
                         std::string filename, int begin_line, int begin_column,
                         int end_line, int end_column, std::string str_val)
    : m_text(text), m_token(token), m_filename(filename),
      m_begin_line(begin_line), m_begin_column(begin_column),
      m_end_line(end_line), m_end_column(end_column) {
 ...
 */
std::string SyntaxToken::text() const { return m_text; }

/*
 * The type of token this is, represented by the bison generated enum.
 */
token_type_t SyntaxToken::token() const { return m_token; }

/*
 * The filename of where this token is located.
 */
std::string SyntaxToken::filename() const { return m_filename; }

/*
 * The token's starting location line-wise in the file.
 */
int SyntaxToken::begin_line() const { return m_begin_line; }

/*
 * The token's starting location column-wise in the file.
 */
int SyntaxToken::begin_column() const { return m_begin_column; }

/*
 * The token's ending location line-wise in the file.
 */
int SyntaxToken::end_line() const { return m_end_line; }

/*
 * The token's ending location column-wise in the file.
 */
int SyntaxToken::end_column() const { return m_end_column; }

/*
 * The semantic value of the token, stored as a variant type.
 */
token_variant SyntaxToken::semantic_value() const { return m_semantic_value; }
