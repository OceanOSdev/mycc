...
StructDeclarationNode::StructDeclarationNode(
    SyntaxToken *token, std::string id,
    std::vector<VariableGroupDeclarationNode *> vars)
 ...
 */
std::string StructDeclarationNode::struct_name() const { return name; }

/*
 * The members (variables) that this struct contains.
 */
std::vector<VariableGroupDeclarationNode *>
StructDeclarationNode::struct_members() const {
  return members;
}
...
StructDeclarationNode::struct_member_list() const {
  return m_members;
}
