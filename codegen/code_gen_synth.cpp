...
void CodeGenerator::synthesize_init_method() {
    ...
    int local_count = 1;
    int stack_size = 1;
    auto finalized_body = new JVMProcessor::FinalizedBody(init_body, local_count, stack_size);
...
void CodeGenerator::synthesize_clinit_method() {
    ...
    for (auto field : m_fields_needing_clinit) {
        auto expression = Binding::Factory::assignment(field->variable_symbol(), field->initializer());
        statements.push_back(Binding::Factory::statement(expression));
    }

    auto clinit_method_old = look_up_method("<clinit>");
    auto clinit_method_new = clinit_method_old->define_method(Binding::Factory::block(statements));
...
void CodeGenerator::synthesize_j_main_method() {
    ...
    auto finalized_body = new JVMProcessor::FinalizedBody(body, local_count, stack_size);
