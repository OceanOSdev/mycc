...
CodeGenerator::CodeGenerator(CodeGenPayload* payload, bool debug, bool emit_part_four_info) :
    m_payload(payload), 
    m_debug_mode(debug),
...
bool CodeGenerator::emit() {
    ...
    for (auto method : m_methods_to_emit) {
        ...
        (*m_outstream) << indent << jvm_emit::emit_directive_end(JVMProcessor::BlockDirectiveType::CodeDirective) << std::endl;
        (*m_outstream) << jvm_emit::emit_directive_end(JVMProcessor::BlockDirectiveType::MethodDirective) << std::endl;

    }

    (*m_outstream) << jvm_emit::emit_directive_end(JVMProcessor::BlockDirectiveType::ClassDirective) << std::endl;
    return true;
}
    ...
    for (auto decl : m_payload->lowered_global_declarations()) {
        ...
        if (kind == Binding::BoundNodeKind::GlobalStatement) {
            ...
            if (gdec->statement()->kind() == Binding::BoundNodeKind::VariableGroupDeclaration) {
                ...
                auto struct_decl = dynamic_cast<Binding::BoundStructDeclarationNode*>(gdec->statement());
...
void CodeGenerator::resolve_methods() {
    ...
    auto _j_main_method_reference = MethodReference::resolve_method(j_main_symbol, m_payload->filename_base(), true, true);
    ...
    for (auto decl : m_payload->lowered_global_declarations()) {
        if (decl->kind() == Binding::BoundNodeKind::FunctionDefinition) {
            ...
            auto _method_reference = MethodReference::resolve_and_define_method(method, m_payload->filename_base(), true);
...
std::string _mangle_name(std::string method_name, std::string owner, Symbols::FunctionSymbol* function) {
...
MethodReference* CodeGenerator::look_up_method(std::string name, std::string owner) {
