...
private:
  ...
  std::unordered_map<Symbols::FunctionSymbol *, JVMProcessor::FinalizedBody *>
      m_compiled_method_bodies;
   ...
   */
  void
  emit_struct_declaration(Binding::BoundStructDeclarationNode *struct_decl);
  void emit_method(MethodReference *method);
  void emit_field(Binding::BoundVariableDeclarationNode *field,
  ...
  void
  emit_return_statement(Binding::BoundReturnStatementNode *return_statement);
  void emit_goto_statement(Binding::BoundGotoStatementNode *goto_statement);
  void emit_conditional_goto_statement(
  ...
  void emit_variable_group_declaration(
  ...
  void emit_conditional_branch(Binding::BoundExpressionNode *condition,
                               Binding::BoundLabel *&label, bool jmp_if_true);
  void emit_conditional_branch_bin_op(Binding::BoundExpressionNode *&expr,
                                      Binding::BoundLabel *&destination,
                                      bool jmp_if_true, bool &should_repeat);

  static JVMProcessor::JVMOpCode
  get_jump_code(Binding::BoundBinaryOperatorNode *bin_op, bool jmp_if_true,
   ...
   */
  void emit_expression_driver(Binding::BoundExpressionNode *expression,
                              bool used);
  void
  emit_assignment_expression(Binding::BoundAssignmentExpressionNode *assignment,
                             bool used);
  void emit_call_expression(Binding::BoundCallExpressionNode *call_expression,
                            bool used);
  void emit_cast_expression(Binding::BoundCastExpressionNode *cast_expression,
                            bool used);
  void emit_load(Binding::BoundVariableReferenceExpressionNode
                     *variable_reference_expression);
  void emit_store(Binding::BoundVariableReferenceExpressionNode
                      *variable_reference_expression);
  void emit_increment_expression(
      Binding::BoundIncrementExpressionNode *increment_expression, bool used);
  void emit_decrement_expression(
      Binding::BoundDecrementExpressionNode *decrement_expression, bool used);
  void
  emit_binary_expression(Binding::BoundBinaryExpressionNode *binary_expression,
                         bool used);
  void
  emit_unary_expression(Binding::BoundUnaryExpressionNode *unary_expression,
                        bool used);
   ...
   * goes unused.
   *
   * @param used Whether or not the result from the current instruction will be
   * used.
   */
  ...
  /// TODO: should probably refactor these methods into their own class or
  /// something.
...
public:
  CodeGenerator(CodeGenPayload *payload, bool debug,
                bool emit_part_four_info = false);
