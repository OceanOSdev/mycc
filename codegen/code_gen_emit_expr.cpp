...
void CodeGenerator::emit_expression(Binding::BoundExpressionNode *expression,
                                    bool used) {
  if (expression == nullptr)
    return; // I don't believe we should ever hit this case, but there might be
            // an edge case I missed.

  // Let's get the case that we're dealing with a constant expression out of the
  // way.
  if (expression->kind() == Binding::BoundNodeKind::LiteralExpression) {
    if (!used)
      return; // If we're not using it there's no reason to pollute the stack
              // with it.
    auto lit =
        dynamic_cast<Binding::BoundLiteralValExpressionNode *>(expression);
    auto type = lit->type();
    if (Symbols::TypeSymbol::are_types_equal(&Symbols::TypeSymbol::Char,
                                             type)) {
      char c_val = lit->char_value();
      m_builder->emit_constant((int)c_val);
    } else if (Symbols::TypeSymbol::are_types_equal(&Symbols::TypeSymbol::Int,
                                                    type))
      m_builder->emit_constant(lit->int_value());
    else if (Symbols::TypeSymbol::are_types_equal(&Symbols::TypeSymbol::Float,
                                                  type))
      ...
      auto args =
          new JVMProcessor::MethodCallInstructionArgument(owner, symbol);
...
void CodeGenerator::emit_expression_driver(
    Binding::BoundExpressionNode *expression, bool used) {
  using namespace Binding;
  auto kind = expression->kind();
  switch (kind) {
  case BoundNodeKind::AssignmentExpression:
    emit_assignment_expression(
        dynamic_cast<BoundAssignmentExpressionNode *>(expression), used);
    break;
  case BoundNodeKind::CallExpression:
    emit_call_expression(dynamic_cast<BoundCallExpressionNode *>(expression),
                         used);
    break;
  case BoundNodeKind::CastExpression:
    emit_cast_expression(dynamic_cast<BoundCastExpressionNode *>(expression),
                         used);
    break;
  ...
  case BoundNodeKind::UnaryExpression:
    break;
  default:
    throw new std::runtime_error("Invalid expression type while emitting: " +
                                 std::to_string(kind));
  }
}

void CodeGenerator::emit_assignment_expression(
    Binding::BoundAssignmentExpressionNode *assignment, bool used) {
  emit_expression(assignment->expression(), true);
  // in the event we have an expression like 'int a = (b = 4)' we need to
  // duplicate (b=4) on the stack so that later code gen has access to it.
  if (used)
    m_builder->emit_op_code(JVMProcessor::JVMOpCode::dup);
  emit_store(assignment->variable_reference());
}

void CodeGenerator::emit_call_expression(
    Binding::BoundCallExpressionNode *call_expression, bool used) {
  ...
  auto args = new JVMProcessor::MethodCallInstructionArgument(method->owner(),
                                                              method->method());
  int stack_delta = calculate_call_stack_delta(call_expression->function());
  m_builder->emit_op_code(op_code, args, stack_delta);

  // check to see if we need to clean up the stack if a nonvoid method's return
  // is not used.
  if (!Symbols::TypeSymbol::is_error_or_incomplete_type(
          call_expression->type())) {
    emit_pop_if_unused(used);
  }
}

void CodeGenerator::emit_cast_expression(
    Binding::BoundCastExpressionNode *cast_expression, bool used) {
  ...
  // dealing with i2f, f2i, i2b (not even really), and i2c which have all unique
  // targets unless of course I fucked up somewhere...
...
void CodeGenerator::emit_load(Binding::BoundVariableReferenceExpressionNode
                                  *variable_reference_expression) {
  auto ref = variable_reference_expression->variable_reference();
  /// TODO: if variable is constant and its val is a LVE then just push the
  /// value to the stack instead of emitting a load instruction.
...
void CodeGenerator::emit_store(Binding::BoundVariableReferenceExpressionNode
                                   *variable_reference_expression) {
  auto ref = variable_reference_expression->variable_reference();
...
void CodeGenerator::emit_increment_expression(
    __attribute__((
        unused)) Binding::BoundIncrementExpressionNode *increment_expression,
    __attribute__((unused)) bool used) {}

void CodeGenerator::emit_decrement_expression(
    __attribute__((
        unused)) Binding::BoundDecrementExpressionNode *decrement_expression,
    __attribute__((unused)) bool used) {}

void CodeGenerator::emit_binary_expression(
    __attribute__((
        unused)) Binding::BoundBinaryExpressionNode *binary_expression,
    __attribute__((unused)) bool used) {}

void CodeGenerator::emit_unary_expression(
    __attribute__((unused)) Binding::BoundUnaryExpressionNode *unary_expression,
    __attribute__((unused)) bool used) {}
