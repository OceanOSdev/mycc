...
void CodeGenerator::emit_statement(Binding::BoundStatementNode *statement) {
  ...
  case BoundNodeKind::ConditionalGotoStatement:
    emit_conditional_goto_statement(
  ...
  case BoundNodeKind::VariableGroupDeclaration:
    emit_variable_group_declaration(
        dynamic_cast<BoundVariableGroupDeclarationNode *>(statement));
    break;
  case BoundNodeKind::
      VariableDeclaration: // <-- gets handled above but I figured I'd add it in
                           // anyways just in case I miss some edge case where
                           // we recurse and hit this unexpectedly.
  ...
  case BoundNodeKind::ExpressionStatement:
    emit_expression(
        dynamic_cast<BoundExpressionStatementNode *>(statement)->expression(),
        false);
    break;
  default:
    throw new std::runtime_error("Invalid statement type while emitting: " +
                                 std::to_string(kind));
  }
}

void CodeGenerator::emit_block_statement(
...
void CodeGenerator::emit_return_statement(
  ...
  if (return_statement->expression() != nullptr) {
    ...
    if (Symbols::TypeSymbol::is_bytecode_reference_type(type)) {
      ...
      bool is_int_ret = Symbols::TypeSymbol::are_types_equivalent(
          type, &Symbols::TypeSymbol::Int);
      m_builder->emit_return_op_code(is_int_ret
                                         ? JVMProcessor::ReturnOpType::Integer
                                         : JVMProcessor::ReturnOpType::Float);
    }
  } else {
...
void CodeGenerator::emit_goto_statement(
...
void CodeGenerator::emit_conditional_goto_statement(
    Binding::BoundConditionalGotoStatementNode *cond_goto_statement) {
  auto condition = cond_goto_statement->condition();
  auto label = cond_goto_statement->label();
  emit_conditional_branch(condition, label,
                          cond_goto_statement->jump_if_true());
}

void CodeGenerator::emit_label_statement(
    Binding::BoundLabelStatementNode *label_statement) {
  auto label = label_statement->label();
  m_builder->mark_label(label);
}

void CodeGenerator::emit_variable_group_declaration(
...
void CodeGenerator::emit_variable_declaration(
    Binding::BoundVariableDeclarationNode *variable_declaration) {
  m_builder->declare_local(variable_declaration->variable_symbol());
  if (variable_declaration->is_initialized()) {
    auto assignment =
        Binding::Factory::assignment(variable_declaration->variable_symbol(),
                                     variable_declaration->initializer());
    emit_expression(assignment, false);
  }
}

void CodeGenerator::emit_conditional_branch(
    Binding::BoundExpressionNode *condition, Binding::BoundLabel *&label,
    bool jmp_if_true) {
...
void CodeGenerator::emit_conditional_branch_bin_op(
    Binding::BoundExpressionNode *&expr, Binding::BoundLabel *&destination,
    bool jmp_if_true, bool &should_repeat) {
  ...
  case Binding::BoundBinaryOpKind::GreaterThanOrEquals: {
    ...
    JVMProcessor::JVMOpCode op_code =
        get_jump_code(bin_expr->op(), jmp_if_true, reverse_op_code);
    if (destination == nullptr) {
      ...
      std::string cur_stack_str =
          std::to_string(m_builder->current_stack_size());
      std::string op_codes_emitted_str =
          std::to_string(m_builder->instructions_emitted());
      destination =
          new Binding::BoundLabel(cur_stack_str + "_" + op_codes_emitted_str);
    }
...
const int invert_idx_offset =
    4; // could use jvm_op_code_row_length, but this name makes it more clear as
       // to what I'm doing with it.

const j_op_code conditional_jump_op_codes[] = {
    //  <                 //  <=                //  >                 //  >=
    j_op_code::if_icmplt, j_op_code::if_icmple, j_op_code::if_icmpgt,
    j_op_code::if_icmpge, // regular
    j_op_code::if_icmpge, j_op_code::if_icmpgt, j_op_code::if_icmple,
    j_op_code::if_icmplt, // reverse
    j_op_code::iflt, j_op_code::ifle, j_op_code::ifgt,
    j_op_code::ifge, // regular (float)
    j_op_code::ifge, j_op_code::ifgt, j_op_code::ifle,
    j_op_code::iflt, // reverse (float)

    //  ==                //  !=                // two repetitions to pad the
    //  row so that the rev offset can remain consistent
    j_op_code::if_icmpeq, j_op_code::if_icmpne, j_op_code::if_icmpeq,
    j_op_code::if_icmpne, // regular
    j_op_code::if_icmpne, j_op_code::if_icmpeq, j_op_code::if_icmpne,
    j_op_code::if_icmpeq, // reverse
    j_op_code::ifeq, j_op_code::ifne, j_op_code::ifeq,
    j_op_code::ifne, // regular (float)
    j_op_code::ifne, j_op_code::ifeq, j_op_code::ifne,
    j_op_code::ifeq, // regular (float)
};

JVMProcessor::JVMOpCode
CodeGenerator::get_jump_code(Binding::BoundBinaryOperatorNode *bin_op,
                             bool jmp_if_true,
                             JVMProcessor::JVMOpCode &reverse_op_code) {
  int op_idx;
  switch (bin_op->op_kind()) {
  case Binding::BoundBinaryOpKind::Equals:
    op_idx = eq_cmp_offset;
    break;
  case Binding::BoundBinaryOpKind::NotEquals:
    op_idx = eq_cmp_offset + 1;
    break;
  case Binding::BoundBinaryOpKind::LessThan:
    op_idx = order_cmp_offset;
    break;
  case Binding::BoundBinaryOpKind::LessThanOrEquals:
    op_idx = order_cmp_offset + 1;
    break;
  case Binding::BoundBinaryOpKind::GreaterThan:
    op_idx = order_cmp_offset + 2;
    break;
  case Binding::BoundBinaryOpKind::GreaterThanOrEquals:
    op_idx = order_cmp_offset + 3;
    break;
  default:
    throw new std::runtime_error("Invalid jump code");
  }
  ...
  if (TypeSymbol::are_types_equal(bin_op->type(), &TypeSymbol::Float)) {
    op_idx +=
        4 *
        jvm_op_code_row_length; // the equivalent comparison op for floats is 16
                                // elements ahead of the original offset.
  }

  int reverse_op_idx = op_idx;
  if (!jmp_if_true)
    op_idx += invert_idx_offset;
  else
    reverse_op_idx += invert_idx_offset;
