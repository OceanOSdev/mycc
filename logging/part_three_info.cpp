...
void PartThreeInfoList::add_var_info(std::string _type, std::string _name, bool init, bool global) {
    if (ignore) return;
...
void PartThreeInfoList::struct_builder_add_member(std::string _type, std::string _name) {
...
void PartThreeInfoList::func_builder_add_param(std::string _type, std::string _name) {
    if (ignore) return;
    PartThreeVariableInfo* var_info = new PartThreeVariableInfo(_type, _name, false);
    m_func_builder->param_info.push_back(var_info);
}

void PartThreeInfoList::func_builder_add_statement(int lineno, std::string stmt_type) {
    if (ignore) return;
