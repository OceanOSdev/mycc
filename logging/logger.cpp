...
void Logger::log_info(std::string str) { (*m_outstream) << str; }
...
void Logger::log_diagnostic(Diagnostic *diagnostic) {
  ...
  errstream << "Error near " << diagnostic->location().file_name << " line "
...
void Logger::log_lex_info(char *filename, int lineNum, char *text,
                          token_type token) {
  (*m_outstream) << filename << " line " << lineNum << " '" << text
                 << "' token " << token_to_string(token) << std::endl;
...
void Logger::log_lex_info_v2(LexemeDataNode lexeme_data) {
  ...
  (*m_outstream) << filename << " line " << bline << " text '"
                 << lexeme_data.text << "' token " << token_name << std::endl;
}

void Logger::log_lex_err(char *errDesc, char *filename, int lineNum,
                         char *text) {
  *(m_is_stdio ? &std::cerr : m_outstream)
      << "Error " << errDesc << " file " << filename << "line " << lineNum;
  // fprintf(ostream, "Error %s file %s line %d", errDesc, filename, lineNum);
  if (text)
    *(m_is_stdio ? &std::cerr : m_outstream)
        << " text '" << text << "'"; // fprintf(ostream, " text \'%s\'", text);
  *(m_is_stdio ? &std::cerr : m_outstream)
      << std::endl; // fprintf(ostream, "\n");
}

void Logger::log_lex_err_v2(LexemeDataNode lexeme_data) {
  std::string filename = *(lexeme_data.symbol_info.location.begin.filename);
  yy::position::counter_type line_num =
      lexeme_data.symbol_info.location.begin.line;
  *(m_is_stdio ? &std::cerr : m_outstream)
      << "Error " << lexeme_data.err_desc << " file " << filename << " line "
      << line_num;
  if (!lexeme_data.text.empty())
    *(m_is_stdio ? &std::cerr : m_outstream)
        << " text '" << lexeme_data.text << "'";
