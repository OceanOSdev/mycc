...
BoundBinaryOperatorNode::BoundBinaryOperatorNode(
    BoundBinaryOpKind op, const Symbols::TypeSymbol *left,
    const Symbols::TypeSymbol *right, const Symbols::TypeSymbol *type)
    : m_op_kind(op), m_left_type(left), m_right_type(right), m_type(type) {}

BoundBinaryOperatorNode *
BoundBinaryOperatorNode::BindITypeOperator(BoundBinaryOpKind opKind,
                                           const Symbols::TypeSymbol *left,
                                           const Symbols::TypeSymbol *right) {
  if (!left->attributes().is_integer_type ||
      !right->attributes().is_integer_type)
    return nullptr;

  auto op_ret_type = Symbols::TypeSymbol::get_wider_type(left, right);
  if (op_ret_type == nullptr)
    return nullptr;
...
BoundBinaryOperatorNode *
BoundBinaryOperatorNode::BindNTypeOperator(BoundBinaryOpKind opKind,
                                           const Symbols::TypeSymbol *left,
                                           const Symbols::TypeSymbol *right) {
  if (!left->attributes().is_numeric_type ||
      !right->attributes().is_numeric_type)
    return nullptr;
...
BoundBinaryOperatorNode *BoundBinaryOperatorNode::BindNTypeCompOperator(
    BoundBinaryOpKind opKind, const Symbols::TypeSymbol *left,
    const Symbols::TypeSymbol *right) {
  if (!left->attributes().is_numeric_type ||
      !right->attributes().is_numeric_type)
    return nullptr;
...
BoundBinaryOperatorNode *
BoundBinaryOperatorNode::Bind(Syntax::btokentype syntax_token_type,
                              const Symbols::TypeSymbol *left,
                              const Symbols::TypeSymbol *right) {
 ...
 * The operator kind.
 */
BoundBinaryOpKind BoundBinaryOperatorNode::op_kind() const { return m_op_kind; }
...
BoundBinaryExpressionNode::BoundBinaryExpressionNode(
    BoundBinaryOperatorNode *op, BoundExpressionNode *left,
 ...
 * The bound operator.
 */
BoundBinaryOperatorNode *BoundBinaryExpressionNode::op() const { return m_op; }

/*
 * The bound expression on the left-hand side of the operator.
 */
BoundExpressionNode *BoundBinaryExpressionNode::left() const { return m_left; }
