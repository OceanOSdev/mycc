...
Binder::Binder(Logging::DiagnosticsList *diagnostics, BoundScope *parent)
    : m_diagnostics(diagnostics),
      ...
      m_err_flag(false), m_label_idx(0), m_scope(new BoundScope(parent)),
...
BoundScope *Binder::init_global_scope() {
  ...
  auto _putchar_symbol = Symbols::Factory::function(
      "putchar", &Symbols::TypeSymbol::Int, {&Symbols::TypeSymbol::Int}, true);
  auto _getchar_symbol = Symbols::Factory::simple_function(
      "getchar", &Symbols::TypeSymbol::Int, true);
 ...
 */
Logging::DiagnosticsList *Binder::diagnostics() const { return m_diagnostics; }
 ...
 * during binding.
 */
bool Binder::err_flag() const { return m_err_flag; }

void Binder::set_current_function_scope(
    Symbols::FunctionSymbol *function_symbol) {
...
void Binder::bind_global_declaration(Syntax::GlobalDeclarationNode *gdn) {
  ...
  case Syntax::SyntaxKind::GlobalVariableDeclaration: {
    auto vardec =
        (dynamic_cast<Syntax::GlobalVariableGroupDeclarationNode *>(gdn))
            ->variable_group();
    m_global_decls.push_back(
        new BoundGlobalStatementNode(bind_statement(vardec, true)));
    break;
  }
  case Syntax::SyntaxKind::GlobalStructDeclaration: {
    auto structdec = (dynamic_cast<Syntax::GlobalStructDeclarationNode *>(gdn))
                         ->struct_declaration();
    m_global_decls.push_back(
        new BoundGlobalStatementNode(bind_statement(structdec)));
    break;
  }
  ...
  case Syntax::SyntaxKind::FunctionDefinition: {
    ...
    label_counter = TreeRewriter::rewrite(bound_func_def->statements(),
                                          rewritten, label_counter);
    auto fsymb = bound_func_def->function_symbol();
    if (!Symbols::TypeSymbol::are_types_equivalent(
            fsymb->type(), &Symbols::TypeSymbol::Void) &&
        !FlowAnalysis::ControlFlowGraph::all_paths_in_DAG_return(rewritten)) {
      m_diagnostics->report_all_paths_must_return(func_def->token(),
                                                  fsymb->name());
      m_err_flag = true;
    }
  ...
  default:
    throw std::runtime_error(
        "Unexpected syntax while binding global statement.");
  }
}

Symbols::FunctionSymbol *Binder::bind_function_declaration(
    Syntax::FunctionDeclarationNode *declaration) {
  std::vector<Symbols::ParameterSymbol *> params;
  std::set<std::string>
      param_name_set; // keep track if we've seen a parameter with a given name
  for (auto parameter_syntax : declaration->parameters()) {
    auto ins_res = param_name_set.insert(parameter_syntax->param_name());
    if (!ins_res.second) {
      m_diagnostics->report_redefinition_of_parameter(
          parameter_syntax->token(), parameter_syntax->param_name());
      m_err_flag = true;
      return nullptr;
    }
    ...
    if (Symbols::TypeSymbol::is_error_type(param_type)) {
      if (parameter_syntax->is_struct())
        type_symbol_id =
            "struct " +
            type_symbol_id; // probably not the best way to accomplish this.
      m_diagnostics->report_no_definition_for_type_symbol(
          parameter_syntax->token(), type_symbol_id);
      m_err_flag = true;
      return nullptr;
    }
    ...
    if (is_array)
      parameter_type = parameter_type->as_array_type();
    if (is_const)
      parameter_type = parameter_type->as_const_type();
    params.push_back(new Symbols::ParameterSymbol(
        parameter_syntax->param_name(), parameter_type, is_array, is_const));

    m_part_three_info_list->func_builder_add_param(params.back()->type()->str(),
                                                   params.back()->name());
  }
  ...
  if (Symbols::TypeSymbol::is_error_type(func_type)) {
    m_diagnostics->report_no_definition_for_type_symbol(declaration->token(),
                                                        func_type->str());
    m_err_flag = true;
    return nullptr;
  }
  ...
  return new Symbols::FunctionSymbol(declaration->function_name(), func_type,
                                     params);
}
...
void Binder::bind_function_prototype(Syntax::FunctionPrototypeNode *prototype) {
  ...
  if (!m_err_flag && !m_scope->try_declare_function(symbol)) {
    m_diagnostics->report_conflicting_function_declarations(prototype->token(),
                                                            symbol->name());
    m_err_flag = true;
  }
}

BoundFunctionDefinitionNode *Binder::bind_function_definition(
    Syntax::FunctionDefinitionNode *function_definition) {
  auto symbol =
      bind_function_declaration(function_definition->function_declaration());
  if (m_err_flag)
    return nullptr;
  if (!m_scope->try_declare_function(symbol)) {
    m_diagnostics->report_conflicting_function_declarations(
        function_definition->token(), symbol->name());
    m_err_flag = true;
  ...
  if (func_in_scope->line_defined() != -1) {
    m_diagnostics->report_function_already_defined(function_definition->token(),
                                                   symbol->name());
    m_err_flag = true;
    return nullptr;
  }
  ...
  for (auto param : symbol->params()) {
    m_scope->try_declare_variable(
        Symbols::VariableSymbol::from_parameter_symbol(param));
  }
  ...
  return new BoundFunctionDefinitionNode(
      symbol, dynamic_cast<BoundBlockStatementNode *>(bound_body));
}
