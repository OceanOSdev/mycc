...
void argument_handler(Arguments *args) {
  ...
  case CompilerMode::MODE_ERR:
    break; // <-- should literally never happen.
  case CompilerMode::MODE_ZERO:
    logger->log_info(std::string(version_info));
    break;
  case CompilerMode::MODE_ONE:
    run_lexer(args);
    break;
  case CompilerMode::MODE_TWO:
    run_syntax_checker(args);
    break;
  case CompilerMode::MODE_THREE:
    run_semantic_analyzer(args);
    break;
  case CompilerMode::MODE_FOUR:
    break;
  case CompilerMode::MODE_FIVE:
    break;
  case CompilerMode::MODE_SIX:
    run_syntax_tree_printer(args);
    break;
  case CompilerMode::MODE_SEVEN:
    run_bound_tree_printer(args);
    break;
  case CompilerMode::MODE_EIGHT:
    run_gen_purp_debug(args);
    break;
  }
}
...
void run_bound_tree_printer(Arguments *args) {
  ...
  if (run_parser(args, std::move(d))) {
    ...
    if (binder->err_flag())
      ...
      // quick hack to get the codes filename (since we only support fully
      // compiling one)
      std::string filename = args->input_filenames().at(0);
      printer->print_bound_tree(CodeGen::CodeGenPayload::create_payload(
          filename, binder->global_decls()));
    }
  } else {
    for (auto diagnostic : d.get_diagnostics())
      logger->log_err(diagnostic);
...
void run_gen_purp_debug(Arguments *args) {
  ...
  if (run_parser(args, std::move(d))) {
    ...
    if (binder->err_flag())
      ...
      // compiling one)
      std::string filename = args->input_filenames().at(0);
      auto payload = CodeGen::CodeGenPayload::create_payload(
          filename, binder->global_decls());
