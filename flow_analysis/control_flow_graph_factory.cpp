 ...
 */
Binding::BoundExpressionNode *
negate_expression(Binding::BoundExpressionNode *expr) {
  if (expr->kind() == Binding::BoundNodeKind::LiteralExpression) {
    ...
    return new Binding::BoundLiteralValExpressionNode(
        (int)(!Binding::BoundLiteralValExpressionNode::is_nonzero(
            lit))); // this *shouldn't* bite me in the ass...
                    // maybe I should make a bound node factory
  }
  auto bang_op = Binding::BoundUnaryOperatorNode::Bind(
      Syntax::token_type_t::BANG, expr->type());
  return new Binding::BoundUnaryExpressionNode(bang_op, expr);
}

ControlFlowGraphFactory::ControlFlowGraphFactory()
    : m_start_block(new BasicBlock(true)), m_end_block(new BasicBlock(false)) {}

void ControlFlowGraphFactory::connect_blocks(
    BasicBlock *from, BasicBlock *to, Binding::BoundExpressionNode *condition) {
  Binding::BoundExpressionNode *cond_expr =
      condition; // don't want to mutate argument.
  if (condition != nullptr &&
      condition->kind() == Binding::BoundNodeKind::LiteralExpression) {
    auto lit_expr =
        dynamic_cast<Binding::BoundLiteralValExpressionNode *>(condition);
    if (Binding::BoundLiteralValExpressionNode::is_nonzero(lit_expr))
      cond_expr =
          nullptr; // lit val is true so jump is effectively unconditional.
    else
      return; // lit val is false so jump will never happen, don't connect these
              // blocks.
  }
...
ControlFlowGraph *
ControlFlowGraphFactory::build(std::vector<BasicBlock *> blocks) {
  if (blocks.empty())
  ...
  for (auto block : blocks) {
    for (auto statement : block->statements()) {
      ...
      if (statement->kind() == Binding::BoundNodeKind::LabelStatement)
        m_block_from_label_map.insert(std::make_pair(
  ...
  for (std::vector<BasicBlock *>::size_type i = 0; i < blocks.size(); i++) {
    ...
    for (auto statement : current->statements()) {
      bool is_last_statement_in_block =
          statement == current->statements().back();
      switch (statement->kind()) {
      case Binding::BoundNodeKind::GotoStatement: {
        auto goto_statement =
            dynamic_cast<Binding::BoundGotoStatementNode *>(statement);
        auto destination_block =
            m_block_from_label_map[goto_statement->label()];
        connect_blocks(current, destination_block);
        break;
      }
      case Binding::BoundNodeKind::ConditionalGotoStatement: {
        auto conditional_goto_statement =
            dynamic_cast<Binding::BoundConditionalGotoStatementNode *>(
                statement);
        auto consequence_block =
            m_block_from_label_map[conditional_goto_statement->label()];
        auto alternative_block = next;

        auto negated_condition =
            negate_expression(conditional_goto_statement->condition());
        auto consequence_condition =
            conditional_goto_statement->jump_if_true()
                ? conditional_goto_statement->condition()
                : negated_condition;
        auto alternative_condition =
            conditional_goto_statement->jump_if_true()
                ? negated_condition
                : conditional_goto_statement->condition();
      ...
      case Binding::BoundNodeKind::ReturnStatement:
        connect_blocks(current, m_end_block);
        break;
      case Binding::BoundNodeKind::EmptyStatement:
        [[fallthrough]];
      ...
      case Binding::BoundNodeKind::ExpressionStatement:
        ...
        break;
      default:
        throw std::runtime_error("Unexpected statement while building control "
                                 "flow graph.\n\tStatement type: " +
                                 std::to_string(statement->kind()));
      }
    }
  }
    ...
    for (auto block : blocks) {
      if (block->incoming().empty()) {
        ...
        break; // break out of the for loop and rescan the block list
               /// TODO: Maybe add a diagnostic warning the user that code is
               /// unreachable?
      }
    }
  } while (should_rescan);
...
void ControlFlowGraphFactory::remove_block(
    std::vector<BasicBlock *> &block_list, BasicBlock *block_to_remove) {
  ...
  auto iter_end =
      std::remove(block_list.begin(), block_list.end(), block_to_remove);
