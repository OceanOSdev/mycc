...
ControlFlowGraph::ControlFlowGraph(BasicBlock* start, BasicBlock* end, std::vector<BasicBlock*> blocks, std::vector<BasicBlockBranch*> branches) :
    m_start_block(start),
    m_end_block(end),
    m_block_list(blocks),
    m_branches(branches) {}

BasicBlock* ControlFlowGraph::start_block() const {
    return m_start_block;
}

BasicBlock* ControlFlowGraph::end_block() const {
    return m_end_block;
}
...
ControlFlowGraph* ControlFlowGraph::create(Binding::BoundBlockStatementNode* method_body) {
...
bool ControlFlowGraph::all_paths_in_DAG_return(Binding::BoundBlockStatementNode* method_body) {
    auto graph = create(method_body);

    for (auto branch : graph->end_block()->incoming()) {
        if (branch->from()->statements().empty()) return false;
        auto last_statement = branch->from()->statements().back();
        if (last_statement->kind() != Binding::BoundNodeKind::ReturnStatement) return false;
    }
    return true;
}
