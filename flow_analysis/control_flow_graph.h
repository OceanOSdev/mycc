...
public:
  ...
  create(Binding::BoundBlockStatementNode *method_body);

  /**
   * @brief Determines whether or not all computation paths have a return
   * statement for a given block statement.
   *
   * @param method_body The block statement to check.
   * @return If all paths in the code body have a return statement.
   */
  static bool
  all_paths_in_DAG_return(Binding::BoundBlockStatementNode *method_body);
};
