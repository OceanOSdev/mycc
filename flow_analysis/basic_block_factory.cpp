 ...
 */
void BasicBlockFactory::start_block() { end_block(); }
...
BasicBlockFactory::build(Binding::BoundBlockStatementNode *block) {
  for (auto statement : block->statements()) {
    switch (statement->kind()) {
    case Binding::BoundNodeKind::LabelStatement:
      start_block(); // since we don't add labels to anything other than loops
                     // yet.
    ...
    case Binding::BoundNodeKind::ExpressionStatement:
      ...
      break;
    default:
      throw std::runtime_error("Unexpected statement while analyzing control "
                               "flow.\n\tStatement type: " +
