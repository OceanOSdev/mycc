...
FunctionSymbol *Factory::simple_function(std::string name,
                                         const TypeSymbol *type, bool defined) {
...
FunctionSymbol *Factory::function(std::string name, const TypeSymbol *type,
  ...
  return new FunctionSymbol(
      name, type, params,
      defined ? 1 : -1); // give it an arbitrary line defintion as we only use
                         // it to check if it has been defined.
}
...
FunctionSymbol::FunctionSymbol(std::string name, const TypeSymbol *type,
                               std::vector<ParameterSymbol *> params,
                               int line_defined)
    : Symbol(name), m_type(type), m_params(params),
      m_line_defined(line_defined) {}
 ...
 */
std::vector<ParameterSymbol *> FunctionSymbol::params() const {
  return m_params;
}
 ...
 */
int FunctionSymbol::line_defined() const { return m_line_defined; }

/*
 * Updates this symbol with the line number that the
 * function got defined on.
 */
void FunctionSymbol::set_line_defined(int line) { m_line_defined = line; }
...
bool FunctionSymbol::operator==(const FunctionSymbol &other) const {
  if (std::is_neq(*m_type <=> *other.m_type))
    return false;
