...
const TypeSymbol *Factory::char_type() { return &TypeSymbol::Char; }

const TypeSymbol *Factory::int_type() { return &TypeSymbol::Int; }

const TypeSymbol *Factory::float_type() { return &TypeSymbol::Float; }

const TypeSymbol *Factory::void_type() { return &TypeSymbol::Void; }

TypeAttribute::TypeAttribute(bool is_strct, bool is_int, bool is_num,
                             bool is_arr, bool is_c)
    : is_struct(is_strct), is_integer_type(is_int), is_numeric_type(is_num),
      is_array(is_arr), is_const(is_c) {}

std::partial_ordering
TypeAttribute::operator<=>(const TypeAttribute &other) const {
  ...
  if (is_integer_type == other.is_integer_type &&
      ...
      is_struct == other.is_struct && is_array == other.is_array)
...
std::partial_ordering TypeSymbol::operator<=>(const TypeSymbol &other) const {
  ...
  if (com != std::partial_ordering::equivalent)
    return com;
...
std::string TypeSymbol::str() const {
  std::string ret = "";
  if (m_attributes.is_const)
    ret += "const ";
  if (m_attributes.is_struct)
    ret += "struct ";
  ret = ret + name();
  if (m_attributes.is_array)
    ret += "[]";
  return ret;
}
...
std::string TypeSymbol::jasm_str(bool is_in_method_sig) const {
  ...
  if (!m_attributes.is_struct) {
    if (m_name == "char")
      ...
      return ret + "V"; // the case of an error type should be handled by the
...
const TypeSymbol *TypeSymbol::get_wider_type(const TypeSymbol *lhs,
                                             const TypeSymbol *rhs) {
  auto order = (*lhs <=> *rhs);
  if (order == 0)
    return lhs; // arbitrary
 ...
 */
bool TypeSymbol::are_types_equivalent(const TypeSymbol *lhs,
                                      const TypeSymbol *rhs) {
 ...
 */
bool TypeSymbol::requires_bytecode_cast(const TypeSymbol *from,
                                        const TypeSymbol *to) {
  if (from->attributes().is_array && to->attributes().is_array)
    return (from->name() !=
            to->name()); // if they're same base type, don't need to cast,
                         // otherwise need (invalid) cast
  ...
  if (!(from->attributes().is_numeric_type &&
        to->attributes().is_numeric_type)) {
    return true; // *Technically* would require cast, but such a cast would be
                 // invalid anyways
  }

  return (from->attributes().is_integer_type &&
          !to->attributes().is_integer_type);
}
...
bool TypeSymbol::can_be_explicitly_casted(const TypeSymbol *from,
                                          const TypeSymbol *to) {
  auto order = (*from <=> *to);
...
bool TypeSymbol::is_bytecode_reference_type(const TypeSymbol *type) {
  if (type->attributes().is_array || type->attributes().is_struct ||
...
const TypeSymbol TypeSymbol::String =
    TypeSymbol("char", {false, true, true, true});
