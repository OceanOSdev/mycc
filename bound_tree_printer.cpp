...
void BoundTreePrinter::print_bound_tree(CodeGen::CodeGenPayload *payload) {
  std::cout << "Bound \"Tree\" (for " << payload->output_filename() << ")"
            << std::endl;
  for (auto dec : payload->lowered_global_declarations()) {
    ...
    case Binding::BoundNodeKind::GlobalStatement: {
      ...
      // since (I think (and be warned, I haven't thought about this much)) they
      // shouldn't end up using any conditional statements in them. (i.e. they
    ...
    case Binding::BoundNodeKind::FunctionDefinition: {
      ...
      // label_counter = Binding::TreeRewriter::rewrite(func->statements(),
      // rewritten, label_counter);
      ...
      std::cout << ") : " << func->function_symbol()->type()->str()
                << std::endl;
...
void BoundTreePrinter::print_statement(Binding::BoundStatementNode *statement) {
  ...
  if (kind == Binding::BoundNodeKind::EmptyStatement)
    return;
  ...
  case Binding::BoundNodeKind::ExpressionStatement:
    print_expression_statement(
        dynamic_cast<Binding::BoundExpressionStatementNode *>(statement));
    break;
  case Binding::BoundNodeKind::BlockStatement:
    print_block_statement(
        dynamic_cast<Binding::BoundBlockStatementNode *>(statement));
    break;
  case Binding::BoundNodeKind::VariableGroupDeclaration:
    print_variable_group_declaration(
        dynamic_cast<Binding::BoundVariableGroupDeclarationNode *>(statement));
    break;
  case Binding::BoundNodeKind::VariableDeclaration:
    print_variable_declaration(
        dynamic_cast<Binding::BoundVariableDeclarationNode *>(statement));
    break;
  case Binding::BoundNodeKind::IfStatement:
    print_if_statement(
        dynamic_cast<Binding::BoundIfStatementNode *>(statement));
    break;
  case Binding::BoundNodeKind::ForStatement:
    print_for_statement(
        dynamic_cast<Binding::BoundForStatementNode *>(statement));
    break;
  case Binding::BoundNodeKind::WhileStatement:
    print_while_statement(
        dynamic_cast<Binding::BoundWhileStatementNode *>(statement));
    break;
  case Binding::BoundNodeKind::DoWhileStatement:
    print_do_while_statement(
        dynamic_cast<Binding::BoundDoWhileStatementNode *>(statement));
    break;
  case Binding::BoundNodeKind::ReturnStatement:
    print_return_statement(
        dynamic_cast<Binding::BoundReturnStatementNode *>(statement));
    break;
  case Binding::BoundNodeKind::StructDeclaration:
    print_struct_declaration(
        dynamic_cast<Binding::BoundStructDeclarationNode *>(statement));
    break;
  case Binding::BoundNodeKind::LabelStatement:
    print_label_statement(
        dynamic_cast<Binding::BoundLabelStatementNode *>(statement));
    break;
  case Binding::BoundNodeKind::GotoStatement:
    print_goto_statement(
        dynamic_cast<Binding::BoundGotoStatementNode *>(statement));
    break;
  case Binding::BoundNodeKind::ConditionalGotoStatement:
    print_conditional_goto_statement(
        dynamic_cast<Binding::BoundConditionalGotoStatementNode *>(statement));
    break;
  default:
...
void BoundTreePrinter::print_expression_statement(
    Binding::BoundExpressionStatementNode *expression_statement) {
  print_expression(expression_statement->expression());
}

void BoundTreePrinter::print_block_statement(
    Binding::BoundBlockStatementNode *block_statement) {
  std::cout << '{' << std::endl;
  m_indent++;
...
void BoundTreePrinter::print_variable_group_declaration(
    Binding::BoundVariableGroupDeclarationNode *var_group_decl) {
  bool is_first = true;
...
void BoundTreePrinter::print_variable_declaration(
    Binding::BoundVariableDeclarationNode *var_decl) {
...
void BoundTreePrinter::print_if_statement(
    Binding::BoundIfStatementNode *if_statement) {
  std::cout << "if (";
...
void BoundTreePrinter::print_for_statement(
    Binding::BoundForStatementNode *for_statement) {
  std::cout << "for (";
...
void BoundTreePrinter::print_while_statement(
    Binding::BoundWhileStatementNode *while_statement) {
  std::cout << "while (";
...
void BoundTreePrinter::print_do_while_statement(
    Binding::BoundDoWhileStatementNode *do_while_statement) {
  std::cout << "do";
...
void BoundTreePrinter::print_return_statement(
    Binding::BoundReturnStatementNode *return_statement) {
  std::cout << "return ";
...
void BoundTreePrinter::print_struct_declaration(
    Binding::BoundStructDeclarationNode *struct_declaration) {
  std::cout << "struct " << struct_declaration->struct_symbol()->name()
            << std::endl;
  print_indent();
  std::cout << '{' << std::endl;
  m_indent++;
  ...
  for (auto member : struct_declaration->struct_symbol()->members()) {
    ...
    std::cout << member->name()
              << (member->is_array()
                      ? "[" + std::to_string(member->array_size()) + "] : "
                      : " : ")
              << member->var_type()->str();
  }
  std::cout << std::endl;
...
void BoundTreePrinter::print_label_statement(
    Binding::BoundLabelStatementNode *label_statement) {
  std::cout << label_statement->label()->name() << ":";
}

void BoundTreePrinter::print_goto_statement(
    Binding::BoundGotoStatementNode *goto_statement) {
  std::cout << "goto " << goto_statement->label()->name();
}

void BoundTreePrinter::print_conditional_goto_statement(
    Binding::BoundConditionalGotoStatementNode *conditional_goto_statement) {
...
void BoundTreePrinter::print_expression(
    Binding::BoundExpressionNode *expression) {
  if (expression == nullptr)
    return;
  auto kind = expression->kind();
  switch (kind) {
  case Binding::BoundNodeKind::AssignmentExpression:
    print_assignment_expression(
        dynamic_cast<Binding::BoundAssignmentExpressionNode *>(expression));
    break;
  case Binding::BoundNodeKind::BinaryExpression:
    print_binary_expression(
        dynamic_cast<Binding::BoundBinaryExpressionNode *>(expression));
    break;
  case Binding::BoundNodeKind::CallExpression:
    print_call_expression(
        dynamic_cast<Binding::BoundCallExpressionNode *>(expression));
    break;
  case Binding::BoundNodeKind::CastExpression:
    print_cast_expression(
        dynamic_cast<Binding::BoundCastExpressionNode *>(expression));
    break;
  case Binding::BoundNodeKind::DecrementExpression:
    print_decrement_expression(
        dynamic_cast<Binding::BoundDecrementExpressionNode *>(expression));
    break;
  case Binding::BoundNodeKind::IncrementExpression:
    print_increment_expression(
        dynamic_cast<Binding::BoundIncrementExpressionNode *>(expression));
    break;
  case Binding::BoundNodeKind::IndexExpression:
    print_index_expression(
        dynamic_cast<Binding::BoundIndexExpressionNode *>(expression));
    break;
  case Binding::BoundNodeKind::LiteralExpression:
    print_literal_val_expression(
        dynamic_cast<Binding::BoundLiteralValExpressionNode *>(expression));
    break;
  case Binding::BoundNodeKind::MemberAccessExpression:
    print_member_expression(
        dynamic_cast<Binding::BoundMemberAccessExpressionNode *>(expression));
    break;
  case Binding::BoundNodeKind::VariableReferenceExpression:
    print_var_reference_expression(
        dynamic_cast<Binding::BoundVariableReferenceExpressionNode *>(
            expression));
    break;
  case Binding::BoundNodeKind::TernaryExpression:
    print_ternary_expression(
        dynamic_cast<Binding::BoundTernaryExpressionNode *>(expression));
    break;
  case Binding::BoundNodeKind::UnaryExpression:
    print_unary_expression(
        dynamic_cast<Binding::BoundUnaryExpressionNode *>(expression));
    break;
  default:
    std::cerr << "Unexpected bound node while rewriting expression."
              << std::endl;
  }
}

void BoundTreePrinter::print_assignment_expression(
    Binding::BoundAssignmentExpressionNode *assignment_expression) {
...
void BoundTreePrinter::print_binary_expression(
    Binding::BoundBinaryExpressionNode *binary_expression) {
  std::cout << "(";
...
void BoundTreePrinter::print_call_expression(
    Binding::BoundCallExpressionNode *call_expression) {
...
void BoundTreePrinter::print_cast_expression(
    Binding::BoundCastExpressionNode *cast_expression) {
  std::cout << "(" << cast_expression->type()->str() << ")(";
  print_expression(cast_expression->expression());
  std::cout << ")";
}

void BoundTreePrinter::print_decrement_expression(
    Binding::BoundDecrementExpressionNode *decrement_expression) {
...
void BoundTreePrinter::print_increment_expression(
    Binding::BoundIncrementExpressionNode *increment_expression) {
...
void BoundTreePrinter::print_index_expression(
    Binding::BoundIndexExpressionNode *index_expression) {
  std::cout << index_expression->variable_reference()->name() << '[';
  print_expression(index_expression->expression());
  std::cout << ']';
}

void BoundTreePrinter::print_literal_val_expression(
    Binding::BoundLiteralValExpressionNode *literal_val_expression) {
...
void BoundTreePrinter::print_member_expression(
    Binding::BoundMemberAccessExpressionNode *member_expression) {
...
void BoundTreePrinter::print_var_reference_expression(
    Binding::BoundVariableReferenceExpressionNode *var_reference_expression) {
  std::cout << var_reference_expression->variable_reference()->name();
}

void BoundTreePrinter::print_ternary_expression(
    Binding::BoundTernaryExpressionNode *ternary_expression) {
...
void BoundTreePrinter::print_unary_expression(
    Binding::BoundUnaryExpressionNode *unary_expression) {
  print_unary_op_token(unary_expression->op()->op_kind());
  std::cout << " ";
  print_expression(unary_expression->expression());
}

void BoundTreePrinter::print_assignment_type(
    Binding::BoundAssignmentOpKind assignment_token) {
  std::string token = "";
  switch (assignment_token) {
  case Binding::BoundAssignmentOpKind::ASSIGN:
    token = "=";
    break;
...
void BoundTreePrinter::print_binary_op_token(
    Binding::BoundBinaryOpKind binary_op_token) {
  std::string token = "";
  switch (binary_op_token) {
  case Binding::BoundBinaryOpKind::Modulo:
    token = "%";
    break;
  case Binding::BoundBinaryOpKind::BitwiseAnd:
    token = "&";
    break;
  case Binding::BoundBinaryOpKind::BitwiseOr:
    token = "|";
    break;
  case Binding::BoundBinaryOpKind::Addition:
    token = "+";
    break;
  case Binding::BoundBinaryOpKind::Subtraction:
    token = "-";
    break;
  case Binding::BoundBinaryOpKind::Multiplication:
    token = "*";
    break;
  case Binding::BoundBinaryOpKind::Division:
    token = "/";
    break;
  case Binding::BoundBinaryOpKind::Equals:
    token = "==";
    break;
  case Binding::BoundBinaryOpKind::NotEquals:
    token = "!=";
    break;
  case Binding::BoundBinaryOpKind::LessThan:
    token = "<";
    break;
  case Binding::BoundBinaryOpKind::LessThanOrEquals:
    token = "<=";
    break;
  case Binding::BoundBinaryOpKind::GreaterThan:
    token = ">";
    break;
...
void BoundTreePrinter::print_unary_op_token(
    Binding::BoundUnaryOpKind unary_op_token) {
  char token = 0;
  switch (unary_op_token) {
  case Binding::BoundUnaryOpKind::OnesComplement:
    token = '~';
    break;
  case Binding::BoundUnaryOpKind::Negation:
    token = '-';
    break;
  case Binding::BoundUnaryOpKind::LogicalNegation:
    token = '!';
    break;
  }
