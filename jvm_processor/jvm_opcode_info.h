...
/**
 * @note Since it isn't necessary to make a whole class to house the methods
 * pertaining to getting jvm op code metadata we just put them in a nested
 * namespace as to not pollute the @c JVMProcessor @endcode namespace with
 * method signatures.
 */
...
/**
 * @brief Determines if the given op code is a conditional dependent branch
 * instruction.
 *
 * @param op_code The op code.
...
/**
 * @brief Determines if the given op code is a conditional based on a relation
 * between variables.
 *
 * @param op_code The op code.
...
/**
 * @brief Determines how many items the given op code pops off the stack
 * machine.
 *
...
/**
 * @brief Determines how many items the given op code pushes onto the stack
 * machine.
 *
 * @note We can return a signed int8 since the highest fixed stack difference
 *       will only be something around like 4 or something and we use -1 to
...
/**
 * @brief Determines whether the given op code always pushes/pops the same
 * number items.
 *
 * Opcodes like those in the return or invoke family modify the stack in ways
 * that are dependent on their context (e.g. given a stack with n items, return
 * will pop n items) and thus we cannot calculate how they will affect the stack
 * "in a vacuum" so to say.
 *
 * @param op_code The op code.
 * @return True if the net stack modification by 'op_code' is independent from
 * the current state of the stack when executed.
 */
 ...
 *
 * @param op_code The op code.
 * @return The net difference in items on the stack machine after executing 'op
 * code'
 */
