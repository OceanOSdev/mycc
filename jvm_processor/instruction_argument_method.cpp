...
MethodCallInstructionArgument::MethodCallInstructionArgument(
    std::string owner, Symbols::FunctionSymbol *method)
    : m_owner(owner), m_method(method), m_is_type_init(method == nullptr) {}

std::string MethodCallInstructionArgument::owner() const { return m_owner; }
...
std::string MethodCallInstructionArgument::return_type_id() const {
  if (m_is_type_init)
    return Symbols::TypeSymbol::Void.jasm_str();
