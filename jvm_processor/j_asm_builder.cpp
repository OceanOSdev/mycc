...
FinalizedBody::FinalizedBody(std::vector<Instruction *> body, int locals,
                             int max_stack)
    : m_body(body), m_local_count(locals), m_max_stack(max_stack),
      m_is_hard_coded(false) {}

FinalizedBody::FinalizedBody(std::vector<std::string> body, int locals,
                             int max_stack)
    : m_hard_coded_body(body), m_local_count(locals), m_max_stack(max_stack),
      m_is_hard_coded(true) {}

std::vector<Instruction *> FinalizedBody::body() const { return m_body; }

std::vector<std::string> FinalizedBody::hard_coded_body() const {
  return m_hard_coded_body;
}

bool FinalizedBody::is_hard_coded() const { return m_is_hard_coded; }

int FinalizedBody::local_count() const { return m_local_count; }

int FinalizedBody::max_stack_size() const { return m_max_stack; }
...
JAsmBuilder::JAsmBuilder()
    : m_builder_state(new BuilderState()), m_finalized(false) {}
...
void JAsmBuilder::emit_op_code(JVMOpCode op_code, int stack_adj) {
  if (OpCodeInfo::required_argument_kind(op_code) !=
      InstructionArgumentKind::Empty)
    throw new std::runtime_error("Invalid number of arguments for op code '" +
                                 OpCodeInfo::op_code_name(op_code) + "'");

  if (OpCodeInfo::is_control_transfer(op_code))
    throw new std::runtime_error(
        "Do not emit control transferring op codes directly");
...
void JAsmBuilder::emit_op_code(JVMOpCode op_code, InstructionArgument *arg,
  ...
  if (OpCodeInfo::required_argument_kind(op_code) != arg->kind())
    throw new std::runtime_error("Invalid argument for op code '" +
                                 OpCodeInfo::op_code_name(op_code) + "'");
...
void JAsmBuilder::emit_return_op_code(ReturnOpType ret_op_type) {
  ...
  JVMOpCode op_code;
  switch (ret_op_type) {
  case ReturnOpType::Void:
    op_code = JVMOpCode::_return;
    break;
  case ReturnOpType::Integer:
    op_code = JVMOpCode::ireturn;
    break;
  case ReturnOpType::Float:
    op_code = JVMOpCode::freturn;
    break;
  case ReturnOpType::Object:
    op_code = JVMOpCode::areturn;
    break;
  default:
    throw new std::runtime_error(
        "Invalid ret operator type"); // <-- should never happen.
  }
...
void JAsmBuilder::emit_branch_op_code(JVMOpCode op_code,
  ...
  if (op_code == JVMOpCode::nop)
    return;

  auto label_arg = dynamic_cast<LabelInstructionArgument *>(arg);
  mark_branch_for_fix_up(m_builder_state->op_codes_emitted(),
                         label_arg->bound_label());
  record_instruction(new Instruction(op_code, arg));
  // m_builder_state->add_op_code(op_code);
  // m_instructions.push_back(new Instruction(op_code, arg));
}
...
void JAsmBuilder::emit_local_load(Symbols::VariableSymbol *local) {
  ...
  // assert(((local_index >= 0), "Local variable is not declared in current
  // scope, cannot emit load."));

  using namespace Symbols;
  if (TypeSymbol::is_bytecode_reference_type(local->var_type()))
    emit_local_load_or_store_reference(local_index);
  else if (TypeSymbol::are_types_equivalent(local->var_type(),
                                            &TypeSymbol::Int))
...
void JAsmBuilder::emit_local_load_or_store_int(int local_index, LSFlag flag) {
  ...
  switch (local_index) {
  case 0:
    emit_op_code(_emit_load ? JVMOpCode::iload_0 : JVMOpCode::istore_0);
    break;
  case 1:
    emit_op_code(_emit_load ? JVMOpCode::iload_1 : JVMOpCode::istore_1);
    break;
  case 2:
    emit_op_code(_emit_load ? JVMOpCode::iload_2 : JVMOpCode::istore_2);
    break;
  case 3:
    emit_op_code(_emit_load ? JVMOpCode::iload_3 : JVMOpCode::istore_3);
    break;
  default:
    emit_op_code(_emit_load ? JVMOpCode::iload : JVMOpCode::istore,
                 new IConstInstructionArgument(local_index));
  }
}
...
void JAsmBuilder::emit_local_load_or_store_float(int local_index, LSFlag flag) {
  ...
  switch (local_index) {
  case 0:
    emit_op_code(_emit_load ? JVMOpCode::fload_0 : JVMOpCode::fstore_0);
    break;
  case 1:
    emit_op_code(_emit_load ? JVMOpCode::fload_1 : JVMOpCode::fstore_1);
    break;
  case 2:
    emit_op_code(_emit_load ? JVMOpCode::fload_2 : JVMOpCode::fstore_2);
    break;
  case 3:
    emit_op_code(_emit_load ? JVMOpCode::fload_3 : JVMOpCode::fstore_3);
    break;
  default:
    emit_op_code(_emit_load ? JVMOpCode::fload : JVMOpCode::fstore,
                 new IConstInstructionArgument(local_index));
  }
}

void JAsmBuilder::emit_local_load_or_store_reference(int local_index,
                                                     LSFlag flag) {
  bool _emit_load = flag == LSFlag::Load;
  switch (local_index) {
  case 0:
    emit_op_code(_emit_load ? JVMOpCode::aload_0 : JVMOpCode::astore_0);
    break;
  case 1:
    emit_op_code(_emit_load ? JVMOpCode::aload_1 : JVMOpCode::astore_1);
    break;
  case 2:
    emit_op_code(_emit_load ? JVMOpCode::aload_2 : JVMOpCode::astore_2);
    break;
  case 3:
    emit_op_code(_emit_load ? JVMOpCode::aload_3 : JVMOpCode::astore_3);
    break;
  default:
    emit_op_code(_emit_load ? JVMOpCode::aload : JVMOpCode::astore,
                 new IConstInstructionArgument(local_index));
  }
}

void JAsmBuilder::emit_global_load(Symbols::VariableSymbol *global,
                                   std::string containing_class_name) {
  auto instr_arg =
      new FieldAccessorInstructionArgument(containing_class_name, global);

  // Since we're putting a halt on structs, we pretty much can always just call
  // getstatic
  auto op_code = JVMOpCode::getstatic;
  record_instruction(new Instruction(op_code, instr_arg));
}

void JAsmBuilder::emit_global_store(Symbols::VariableSymbol *global,
                                    std::string containing_class_name) {
  auto instr_arg =
      new FieldAccessorInstructionArgument(containing_class_name, global);
...
FinalizedBody *JAsmBuilder::finalize() {
  ...
  return new FinalizedBody(instr_list, m_locals.size(),
                           m_builder_state->max_stack_size());
}
...
void JAsmBuilder::mark_branch_for_fix_up(int instruction_idx,
                                         Binding::BoundLabel *label) {
...
void JAsmBuilder::run_label_fixes() {
  ...
  for (auto fixup : m_fixups) {
    ...
    existing_labels.push_back(
        dynamic_cast<LabelInstructionArgument *>(inst_to_fix->argument()));

    calculated_label_pos.push_back(target_instruction_idx);
    //((LabelInstructionArgument*)(inst_to_fix->argument()))->bound_label()->m_name
    //= "L" + std::to_string(counter++);
  }
