...
enum ReturnOpType : uint8_t { Void, Integer, Float, Object };
...
private:
  ...
  static std::string
  lit_as_emittable_const(Binding::BoundLiteralValExpressionNode *lit);
   ...
   */
  enum LSFlag : uint8_t { Load, Store };
  ...
  void emit_local_load_or_store_int(int local_index,
                                    LSFlag flag = LSFlag::Load);
  void emit_local_load_or_store_float(int local_index,
                                      LSFlag flag = LSFlag::Load);
  void emit_local_load_or_store_reference(int local_index,
                                          LSFlag flag = LSFlag::Load);
...
public:
   ...
   * create a psuedo-struct with the name of the source file (w/o the file
   * extension) so that we can access the static field of emitted "class".
   *
   * @param global The global variable symbol.
   * @param containing_class_name The base filename of the source file that
   * defines the global.
   */
  void emit_global_load(Symbols::VariableSymbol *global,
                        std::string containing_class_name);
  void
  emit_global_store(Symbols::VariableSymbol *global,
                    std::string containing_class_name); // <-- similar situation
   ...
   * @param requires_clinit An out-param that notifies the caller if the field
   * needs to be initialized in the <clinit> method.
   * @return '.field public [static] [final] <name> <Type> [= <init_expr>]'
   */
  static std::string
  emit_field_directive(Binding::BoundVariableDeclarationNode *field_decl,
                       bool is_static, bool &requires_clinit);

  /**
   * @brief Emits a string representation of the method directive.
   *
   * @param method The function symbol for this method.
   * @param is_static Whether or not this is a special function (i.e an
   * initializer).
   * @return '.method public [static] <name> : ([<Type list>])<Type>'
   */
  static std::string
  emit_method_definition_header(Symbols::FunctionSymbol *method,
                                bool is_static = true);
   ...
   */
  static std::string emit_method_code_directive(int max_stack_size,
                                                int local_count);
