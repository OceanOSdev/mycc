...
int8_t stack_pop_modifier(JVMOpCode op_code) {
  ...
  case JVMOpCode::iload_1:
    [[fallthrough]];
  case JVMOpCode::iload_2:
    [[fallthrough]];
  case JVMOpCode::iload_3:
    [[fallthrough]];
  case JVMOpCode::fload_0:
    [[fallthrough]];
  case JVMOpCode::fload_1:
    [[fallthrough]];
  case JVMOpCode::fload_2:
    [[fallthrough]];
  case JVMOpCode::fload_3:
    [[fallthrough]];
  ...
  case JVMOpCode::istore_1:
    [[fallthrough]];
  case JVMOpCode::istore_2:
    [[fallthrough]];
  case JVMOpCode::istore_3:
    [[fallthrough]];
  case JVMOpCode::fstore_0:
    [[fallthrough]];
  case JVMOpCode::fstore_1:
    [[fallthrough]];
  case JVMOpCode::fstore_2:
    [[fallthrough]];
  case JVMOpCode::fstore_3:
    [[fallthrough]];
  ...
  case JVMOpCode::pop2:
    return 2;
  case JVMOpCode::dup:
    return 1; // pops off before pushing two back on
  case JVMOpCode::dup2:
    return 2; // pops two off before pushing them back on twice
  case JVMOpCode::swap:
    return 2; // pops two off before reversing and pushing them back on
  ...
  case JVMOpCode::fcmpl:
    [[fallthrough]];
  case JVMOpCode::fcmpg:
    return 2;
  ...
  case JVMOpCode::ireturn:
    [[fallthrough]];
  case JVMOpCode::freturn:
    [[fallthrough]];
  case JVMOpCode::areturn:
    [[fallthrough]];
  case JVMOpCode::_return:
    return -1; // empties the stack.
  ...
  case JVMOpCode::invokestatic:
    return -1; // can't be arbitrarily long like some others, but since it could
               // be either 0 or 1 we consider it variable
  ...
  case JVMOpCode::anewarray:
    return 1; // pops the size of the array before creating it

  case JVMOpCode::arraylength:
    return 1; // pops ref before pushing length
  default:
    throw new std::runtime_error("Invalid op code value: " +
...
int8_t stack_push_modifier(JVMOpCode op_code) {
  ...
  case JVMOpCode::areturn:
    [[fallthrough]];
  case JVMOpCode::_return:
    return 0; // doesn't push anything
...
int net_stack_modification(JVMOpCode op_code) {
  assert((is_constant_stack_modifier(op_code),
          "Cannot determine net stack modification of variable modifier."));
  return stack_push_modifier(op_code) - stack_pop_modifier(op_code);
}

bool is_control_transfer(JVMOpCode op_code) {
  if (is_branch(op_code))
    return true;
...
InstructionArgumentKind required_argument_kind(JVMOpCode op_code) {
  ...
  case JVMOpCode::sipush:
    return InstructionArgumentKind::Integer_Constant; // we'll still emit sipush

  case JVMOpCode::ldc:
    return InstructionArgumentKind::Any_Constant;
