...
const std::set<std::string> JAsmBuilder::m_forbidden_words{
    ...
    "super",     "synchronized", "open",      "transitive", "volatile",
    "bridge",    "static_phase", "transient", "varargs",    "native",
    "interface", "abstract",     "strict",    "synthetic",  "annotation",
    "enum",      "module",       "mandated",
    "strictfp" // <-- Krakatau assembly also disallows this one too, so we add
               // it to our "forbidden" words.
};

std::string JAsmBuilder::lit_as_emittable_const(
    Binding::BoundLiteralValExpressionNode *lit) {
  auto type = lit->type();
  ...
  if (TypeSymbol::are_types_equal(&TypeSymbol::Float, type))
    return (lit->float_value() > 0 ? "+" : "") +
           std::to_string(lit->float_value()) + "f";
  if (TypeSymbol::are_types_equal(&TypeSymbol::String, type))
    return "'" + lit->string_value() + "'";
  return "??"; // we know its a lit when we call this function so just return
               // whatever.
}
...
std::string JAsmBuilder::emit_field_directive(
    Binding::BoundVariableDeclarationNode *field_decl, bool is_static,
    bool &requires_clinit) {
  ...
  if (field_decl->is_initialized()) {
    if (is_const && field_decl->initializer()->kind() ==
                        Binding::BoundNodeKind::LiteralExpression) {
      auto literal = dynamic_cast<Binding::BoundLiteralValExpressionNode *>(
          field_decl->initializer());
      if (Symbols::TypeSymbol::are_types_equal(literal->type(),
...
std::string
JAsmBuilder::emit_method_definition_header(Symbols::FunctionSymbol *method,
                                           bool is_static) {
...
std::string JAsmBuilder::emit_method_code_directive(int max_stack_size,
                                                    int local_count) {
  return ".code stack " + std::to_string(max_stack_size) + " locals " +
