...
enum InstructionArgumentKind : uint16_t {
  ...
  // Helper vals
  Any_Constant =
      Char_Constant | Integer_Constant | Float_Constant | String_Constant
};
...
public:
  FieldAccessorInstructionArgument(Symbols::StructSymbol *_struct,
                                   Symbols::VariableSymbol *member);
  /**
   * @brief Field accessor instruction argument in the event that we're actually
   * just accessing a global variable in the original source code.
   *
   * @param filename_base The base filename (no extensions) of the global
   * variable container.
   * @param member The variable symbol of the global variable.
   */
  FieldAccessorInstructionArgument(std::string filename_base,
                                   Symbols::VariableSymbol *member);
...
public:
  MethodCallInstructionArgument(std::string owner,
                                Symbols::FunctionSymbol *method);
