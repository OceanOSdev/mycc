...
public:
  ...
  std::string err_desc;
  int err_line_num;
  LexemeDataNode(yy::parser::symbol_type st, yyparsetoken_t t, std::string txt,
                 bool err, std::string err_d, int er_l_n = -1)
      : symbol_info(st), token(t), text(txt), is_error(err), err_desc(err_d),
        err_line_num(er_l_n) {}
};
